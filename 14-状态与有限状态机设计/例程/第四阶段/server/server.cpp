...
int main(int argc, char* argv[])
{
	...
	if(ns <= 0)
		return 0;
	closesocket(s);
	ProgramLog("accept a connection",NULL);
		...
		if(retval == 0){
			...
			if(recvbuf.type == FRAME_DATA){
				if(recvbuf.sequence_num == 0){
					ProgramLog("Recv frame 0",NULL);
					event = EVENT_RECV0;
				}
				else if(recvbuf.sequence_num == 1){
					event = EVENT_RECV1;
					ProgramLog("Recv frame 1",NULL);
				}
			}
			else
				continue;
...
int Start_fsm(int event)
{
	...
	case EVENT_RECV0:
		...
		if(error_simulator(PROB) == FALSE){
			...
			ProgramLog("Send ACK",NULL);
		}else
			ProgramLog("Send ACK","But lost");
...
int R0_fsm(int event)
{
	...
	case EVENT_RECV0:
		//duplicate frame
		ProgramLog("Dicard frame 0","Duplicate");
		//send ack
		sendbuf.type = FRAME_ACK;
		sendbuf.len = 3;
